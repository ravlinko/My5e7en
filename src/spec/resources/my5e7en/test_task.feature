Feature: Seven company test task requirements
  ...
  Scenario: admin should have access to all tabs
    ...
    When admin user logged in with password demo
  ...
  Scenario: available application security roles of user
    Given a login page is opened
    And admin user logged in with password demo
  ...
  Scenario: admin user should be able to view companies reports
    ...
    Then reports table is
      | Green Garden inc. 2017 May month report | Green Garden inc. | Derek Tailor  |
      | Red Dragon inc. 2017 May month report   | Red Dragon inc.   | Yakito Sunoro |
      | Chery Street inc. 2017 May month report | Chery Street inc. | Mike Sonner   |
